...
**The stage block pages under `public/content/stages/stages-*.md` are GENERATED. Do not hand-edit them** - each opens with a `<!-- Generated by ... -->` comment directly under the frontmatter, and the next regeneration overwrites any manual change. Their prose lives in `scripts/stage-pages.config.json`.

The marker is a body comment rather than a `generated: true` frontmatter key on purpose. The in-browser page editor validates frontmatter against a fixed allowlist (`id`, `title`, `description`, `tags`, `category`, `date`, `order`, `background` - `wiki-framework/src/components/wiki/PageEditor.jsx`) and **blocks saving** on any other key, so custom frontmatter silently makes a page uneditable for contributors. Never add a new frontmatter field to generated or hand-written content without first extending that allowlist in the framework repo. (`public/content/home.md` still trips this with `hideHeader`, which `PageViewer` supports but the editor rejects.)
