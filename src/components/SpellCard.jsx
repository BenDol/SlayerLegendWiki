...
import { getSpellImage, getGenericSkillIcon } from '../services/imageService';

/**
 * SpellCard component - Displays spell/skill information in a card format
 ...
 * <SpellCard name="Fire Slash" mode="compact" />
 * <SpellCard name="Fire Slash" mode="detailed" />
 * <SpellCard name="Fire Slash" mode="advanced" />
 *
 * @param {string} mode - Display mode: 'compact', 'detailed' (default), 'advanced'
 */
const SpellCard = ({ id, name, spell, mode = 'detailed' }) => {
  const [spellData, setSpellData] = React.useState(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState(null);
  const [spellImage, setSpellImage] = React.useState(getGenericSkillIcon());

  React.useEffect(() => {
    // If spell object provided directly, use it
    if (spell) {
      setSpellData(spell);
      setLoading(false);
      return;
    ...
    const loadSpell = async () => {
      try {
        ...
        // Find spell by id or name
        let foundSpell;
        if (id !== undefined) {
          foundSpell = skills.find(s => s.id === parseInt(id));
        } else if (name) {
          foundSpell = skills.find(s => s.name === name);
        }

        if (!foundSpell) {
          throw new Error(`Spell not found: ${name || `ID ${id}`}`);
        }

        setSpellData(foundSpell);
        setLoading(false);
      } catch (err) {
  ...
  const attributeGradient = attributeColors[spellData.attribute] || 'from-gray-500 to-gray-600';
  const gradeColor = gradeColors[spellData.grade] || 'bg-gray-500';

  // Calculate damage at max level
  const maxLevelDamage = spellData.baseValue + (spellData.upgradeValue * (spellData.maxLevel - 1));
      ...
      <div className="not-prose inline-flex items-center gap-2 bg-white dark:bg-gray-800 rounded-md shadow-sm border border-gray-200 dark:border-gray-700 px-2 py-1.5 my-1 max-w-fit">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <h4 className="font-semibold text-sm text-gray-900 dark:text-white truncate m-0">{spellData.name}</h4>
            <span className={`${gradeColor} text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0`}>
              {spellData.grade}
            </span>
          </div>
          <div className="flex items-center gap-2 text-[10px] text-gray-600 dark:text-gray-400">
            <span>💧 {spellData.mpCost}</span>
            <span>⏱️ {spellData.cooldown}s</span>
            <span>📏 {spellData.range === 0 ? 'Self' : spellData.range}</span>
          </div>
        </div>
      ...
      <div className="not-prose bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 my-2">
        ...
        <div className={`bg-gradient-to-r ${attributeGradient} p-2.5 text-white`}>
          <div className="flex items-start gap-3">
            ...
            {/* Spell Info */}
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold mb-0.5">{spellData.name}</h3>
              <p className="text-xs opacity-90 line-clamp-2">{spellData.basicDescription}</p>
            </div>
        ...
        <div className="p-3 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded p-1.5 text-center">
              ...
              <div className="text-sm font-bold text-green-600 dark:text-green-400">
                {spellData.range === 0 ? 'Self' : spellData.range}
              </div>
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded p-2 flex items-center justify-between text-xs">
            <div>
              ...
              <span className="font-bold text-gray-800 dark:text-gray-200">+{spellData.upgradeValue}%</span>
            </div>
            <div>
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded p-2">
            ...
            <div className="grid grid-cols-5 gap-1 text-[10px]">
              {[1, Math.floor(spellData.maxLevel / 4), Math.floor(spellData.maxLevel / 2), Math.floor(spellData.maxLevel * 3 / 4), spellData.maxLevel].map((level) => {
                const damage = spellData.baseValue + (spellData.upgradeValue * (level - 1));
                return (
    ...
    <div className="not-prose bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 my-2">
      ...
      <div className={`bg-gradient-to-r ${attributeGradient} p-2 text-white`}>
        <div className="flex items-center gap-2">
          ...
          {/* Spell Info */}
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold">{spellData.name}</h3>
            <p className="text-xs opacity-90 truncate">{spellData.basicDescription}</p>
          </div>
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
          <span>📏 Range: {spellData.range === 0 ? 'Self' : spellData.range}</span>
          <span>🔓 Lv {spellData.enterLevel}</span>
          <span>⬆️ Max Lv {spellData.maxLevel}</span>
        </div>
      </div>
    </div>
  );
