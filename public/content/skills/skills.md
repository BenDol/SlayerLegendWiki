---
id: skills
title: Skills
description: A brief overview of the skill system.
tags:
  - skills
category: Skills
date: '2026-01-05'
order: 0
---
# <span class="text-gray-900 dark:text-gray-100">Skills Overview</span>
Aside from your normal attacks, there are skills that deal more damage or help amplify combat and non-combat stats.


# <span class="text-gray-900 dark:text-gray-100">Common Skills</span>
Your starting toolkit, unlocking within the first few character levels: the four elemental attacks (Fire Slash, Ice Stone, Lightning Slash, Stone Strike) plus the first buffs - Fire Sword for ATK, Mana's Blessing for mana recovery, and Ground's Blessing for healing. Full numbers for every skill are in the [Skill Database](/database/skills).

<img src="/images/content/skills/icons/00_FlameSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/00_IceStone.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/00_StoneStrike.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/04_ManaRecovery_1.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/12_EarthCount.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/07_BurningWeapon_1.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/LightningSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Great Skills</span>
The first step up: Lightning Stroke rains 30 strikes across the field, Hot Blast and Ice Shower widen your area damage, Power Strike doubles up on earth damage, Power Impact adds a stunning shockwave, and Agile speeds up your movement between packs.

<img src="/images/content/skills/icons/LightningStroke.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/01_IceShower.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/03_FireWind.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/09_Tornado.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/01_PowerStrike.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/03_PowerWave.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Rare Skills</span>
Rare grade introduces the ramping passives that anchor builds for the rest of the game - Burning Sword, Speed Sword, and Earth's Will all scale during battle, and Flowing Blade adds burst attack speed. On the active side, Flame Slash, Water Slash, and Thunder Slash escalate the elemental attack lines.

<img src="/images/content/skills/icons/01_BlazeSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/03_IceLancer.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/ThunderSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/08_BurningWeapon_2.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/08_IceAge.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/07_StormBlade.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/08_EarthWill.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Epic Skills</span>
Epic grade is where farming builds are born: Fulgurous - the half-second-cooldown charge - is the backbone of nearly every farming loadout in the community, backed by Flame Wave's splash damage. Iron Will and Wind Sword add permanent ATK and attack speed, Curved Blade ramps in battle, Dancing Waves dodges everything for a few seconds, and Life Mana restores HP and mana in one cast.

<img src="/images/content/skills/icons/05_BlazeWave.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/09_IceAge 1.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/05_Swiftness.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/IronWill.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/10_ManaRecovery_2.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/11_CycloneBlade.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/10_LifeStock.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Legendary Skills</span>
The heavy artillery. Fire Blast and Giga Impact carry enormous multipliers, Hellfire Slash and Thunderbolt Slash cover huge areas, Giga Strike triples up for boss burst, and Red Lightning saturates the whole screen. The utility pieces matter just as much: Meditation instantly recharges your other cooldowns, and Rage converts missing HP into attack power.

<img src="/images/content/skills/icons/02_ExplosionSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/04_FireWind_Expert.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/02_IceMeteor.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/ThunderboltSlash.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/02_GigaStrike.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/09_Rage.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/05_Meditation.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/04_LightningSwing.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/04_GigaWave.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Mythic Skills</span>
The ultimates, one per element and then some: Pillar of Fire's growing flame columns, Blizzard's full-line freeze, Supersonic's penetrating six-hit charge, and Demon Hunt's seven-strike combo with its massive finisher. The Mythic buffs are dramatic and conditional - Warrior Burn and Lightning Body each consume half your current HP for huge temporary power, Strong Current surges attack speed, and Wrath of Gods erupts twenty seconds into battle.

<img src="/images/content/skills/icons/06_ProminenceBurn.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/06_Blizard.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/06_BloodyTornado.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/05_Earthquake.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/10_WarriorBurn.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/StrongCurrent.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/10_WhiteLightning.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/06_Supernatural.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

# <span class="text-gray-900 dark:text-gray-100">Immortal Skills</span>
The top of the ladder, and both break the normal rules. Rave records the damage you deal over five seconds and releases a percentage of it back as bonus damage - it multiplies with every buff you can stack into its window. Mantra boosts total ATK and HP without even occupying a skill slot. Details in the [Skill Database](/database/skills).

<img src="/images/content/skills/icons/16_BlazeDance.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 128px; height: 128px; margin: 0 0.25em;" data-inline="true" />

<img src="/images/content/skills/icons/Rave.png" alt="" class="inline-image" style="display: inline-block; vertical-align: middle; width: 125px; height: 128px; margin: 0 0.25em;" data-inline="true" />
